...
- 默认列表视图，也可切换为图标视图；默认顺序为目录优先、名称自然排序。
- 列表表头可按名称、修改时间或大小循环切换默认顺序、正序和倒序；排序同时应用于图标视图。按大小排序时仅比较文件大小，目录没有递归汇总大小数据，因此始终排在文件之后。
- 面包屑导航、返回上级、刷新、实时搜索和深色/浅色主题。
- 列表和图标视图均使用虚拟滚动；大目录只渲染视口附近的文件项，并保留少量缓冲行，降低数千个文件同时挂载到 DOM 带来的首次渲染和滚动开销。
- 图标视图可展示图片缩略图、视频首帧和音频内嵌封面；图片超过 15 MiB 时仅显示文件图标，避免下载和解码原图造成资源占用。
- 缩略图、视频首帧和音频封面会随卡片进入虚拟窗口后再加载，不会因目录中存在大量媒体文件而同时请求预览资源。
...
- 工具栏多选模式、`Ctrl` / `Cmd` 点选和 `Shift` 连续范围选择；列表和图标视图支持鼠标拖拽圈选。圈选拖到滚动区域顶部或底部时会自动滚动，可跨越虚拟窗口连续选择，反向缩小选区时会同步取消已离开范围的项目。
- 批量下载会在浏览器内合并为一个 ZIP 文件，根目录使用独立的批次名称，避免采用某个已选目录作为压缩包名称。
- 单文件直接下载；文件夹通过 dufs 的 `?zip` 接口下载。
- 可将文件或目录拖到同级目录卡片/行中移动；不能将目录移动到自身或其子目录。
- 支持通过详情面板、批量操作区和列表行菜单发起 Move。可移动单个文件、单个文件夹、多个文件、多个文件夹或文件与文件夹的混合选择。
- Move 弹窗可直接输入目标绝对路径（按 Enter 浏览），也可逐层浏览目录；当前显示的目录即为移动目标。移动会使用 WebDAV `MOVE` 逐项执行，完成后刷新文件列表。

### 文件管理

- 新建空文件和目录。
- 上传单个、多个文件；选择文件夹或拖入文件夹上传。
- 文件夹上传会先递归创建目录，再将文件上传到相应路径；上传队列支持查看、悬停展开和点击固定。启用 `--allow-delete` 时，文件会先上传到临时文件，完成后再原子移动到目标位置；每项末尾均可点击叉号取消，正在上传的临时文件会删除，排队文件会跳过，并从列表移除。
- 重命名、复制、删除文件或目录；批量删除会先确认。
- 纯文本文件可在页面内编辑并保存。编辑功能采用二进制扩展名黑名单，覆盖压缩包、镜像、安装包、办公文档、媒体、数据库和可执行文件等常见格式；无后缀或非标准后缀的文本文件仍可编辑，无后缀文件会额外检测 ELF 文件头，避免把 Linux 可执行文件按文本打开。
...
| 重命名、拖入目录移动、Move 弹窗移动 | `MOVE` | `--allow-upload --allow-delete` |
...
- 所有运行时 CSS 和 JavaScript 都会内联到 `index.html`，兼容 Dufs 的 `--assets` 覆盖机制。
